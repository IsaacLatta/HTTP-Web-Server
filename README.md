...
```
- To stop the server, kernel signals SIGTERM(14) and SIGINT(2) may be sent to the process close the server. These can be sent via control C for SIGINT or the following command for either:
```bash 
pkill -15 web-server
```
## Future Improvements 
